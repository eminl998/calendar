                                            ...
                                            <div class="flex sm:mt-0 sm:flex-row sm:flex-wrap sm:space-x-6">
                                                ...
                                                <div class="flex flex-col font-semibold text-gray-800 dark:text-gray-200 leading-tight">
                                                    ...
                                                    <div style="height: 50px; overflow-y: scroll;">
                                                        <select name="position" class="block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md py-2 px-4">
                                                            <option value="Software Developer" {{old('position')=='Software Developer'?'selected':''}} >Software Developer</option>
                                                            <option value="Front-End Developer" {{old('position')=='Front-End Developer'?'selected':''}} >Front-End Developer</option>
                                                            <option value="Back-End Developer" {{old('position')=='Back-End Developer'?'selected':''}} >Back-End Developer</option>
                                                            ...
                                                            <option value="Quality Assurance (QA) Analyst" {{old('position')=='Quality Assurance (QA) Analyst'?'selected':''}} >Quality Assurance (QA) Analyst</option>
                                                            <option value="User Interface (UI) Designer" {{old('position')=='User Interface (UI) Designer'?'selected':''}} >User Interface (UI) Designer</option>
                                                            <option value="User Experience (UX) Designer" {{old('position')=='User Experience (UX) Designer'?'selected':''}} >User Experience (UX) Designer</option>
                                                            <option value="Technical Writer" {{old('position')=='Technical Writer'?'selected':''}} >Technical Writer</option>
                                                            <option value="Product Manager" {{old('position')=='Product Manager'?'selected':''}} >Product Manager</option>
                                                            <option value="Project Manager" {{old('position')=='Project Manager'?'selected':''}} >Project Manager</option>
                                                            <option value="Technical Support Specialist" {{old('position')=='Technical Support Specialist'?'selected':''}} >Technical Support Specialist</option>
                                                            <option value="Database Administrator" {{old('position')=='Database Administrator'?'selected':''}} >Database Administrator</option>
                                                            <option value="Cloud Architect" {{old('position')=='Cloud Architect'?'selected':''}} >Cloud Architect</option>
                                                            <option value="Security Analyst" {{old('position')=='Security Analyst'?'selected':''}} >Security Analyst</option>
                                ...
                                <tbody>
                                    @foreach ($users as $user)
                                        <tr
                                            ...
                                            <td
                                                class="flex items-center px-8 py-8 text-gray-800 whitespace-nowrap dark:text-white">
                                                <div
                                                    class="font-semibold text-gray-800 dark:text-gray-200 leading-tight">
                                                    {{ $user->id }}
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-20 py-6">
                                                {{ isset($user->daysOff['Annual leave']) && isset($user->daysOff['Parental leave']) && isset($user->daysOff['Sick leave']) && isset($user->daysOff['Compassionate leave']) && isset($user->daysOff['Daily rest']) ? $user->daysOff['Annual leave'] + $user->daysOff['Parental leave'] + $user->daysOff['Sick leave'] + $user->daysOff['Compassionate leave'] + $user->daysOff['Daily rest'] : 0 }}
                                            </td>

                                            <td class="px-20 py-6">
                                                {{ isset($user->daysOff['Annual leave']) ? $user->daysOff['Annual leave'] : 0 }}
                                            </td>

                                            <td class="px-20 py-6">
                                                {{ isset($user->daysOff['Parental leave']) ? $user->daysOff['Parental leave'] : 0 }}
                                            </td>

                                            <td class="px-20 py-6">
                                                {{ isset($user->daysOff['Sick leave']) ? $user->daysOff['Sick leave'] : 0 }}
                                            </td>

                                            <td class="px-20 py-6">
                                                {{ isset($user->daysOff['Compassionate leave']) ? $user->daysOff['Compassionate leave'] : 0 }}
                                            </td>

                                            <td class="px-20 py-6">
                                                {{ isset($user->daysOff['Daily rest']) ? $user->daysOff['Daily rest'] : 0 }}
                                            </td>

                                            <td class="px-10 py-6" style="display: inline-block;">
                                                <div style="display: inline-flex;">
                                                    <a href="https://mail.google.com/mail/?view=cm&to={{ $user->email }}&su=Hello%20from%20my%20website&body=Dear%20{{ $user->name }},%0D%0A%0D%0AThank%20you%20for%20visiting%20my%20website.%20I%20would%20like%20to%20connect%20with%20you%20and%20discuss%20your%20interests%20further.%0D%0A%0D%0ABest%20regards.%0D%0A%0D%0AVianova%20Team"
                                                        target="_blank">
                                                    ...
                                                    <a class="ml-2"
                                                        href="{{ route('users.downloadPdf', ['email' => $user->email]) }}"
                                                        download>
